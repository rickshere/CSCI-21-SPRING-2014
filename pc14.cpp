...
void bubbleSort (int theArray[], unsigned int size) {
  ...
  do {
   ...
   sizeOfUnsortedArray -= 1;
  } while( swapped == true);
}

void swapValues (int values[], int index1, int index2) {
    int temporaryPlaceHolder = values[index2];
    values[index2] = values[index1];
    values[index1] = temporaryPlaceHolder;
}

void selectionSort (int theArray[], unsigned int size) {
    unsigned int minimumValueElement;
    for (unsigned int i=0; i < size - 1; i++) {
	    minimumValueElement = i;
		for (unsigned int j= i + 1; j < size; j++) {
		  if (theArray[j] < theArray[minimumValueElement])
            minimumValueElement = j;
		}
